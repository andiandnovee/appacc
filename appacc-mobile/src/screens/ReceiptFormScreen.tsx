...
export default function ReceiptFormScreen() {
  ...
  const handleSubmit = async () => {
    if (!receiptDate.trim())  return Alert.alert('Perhatian', 'Tanggal wajib diisi');
    ...
    if (!po.company_id)       return Alert.alert('Perhatian', 'Company tidak ditemukan dari data PO');
    if (!po.vendor_id)        return Alert.alert('Perhatian', 'Vendor tidak ditemukan dari data PO');
    ...
    <SafeAreaView style={styles.container}>
      ...
      <ScrollView contentContainerStyle={styles.content} keyboardShouldPersistTaps="handled">
        ...
        <View style={styles.fieldGroup}>
          ...
          {po?.is_pkp && (
            <Text style={styles.fieldHint}>
              Sudah include PPN 11% (DPP: Rp {Number(po.dpp).toLocaleString('id-ID')})
            </Text>
          )}
        </View>
        ...
        <View style={styles.infoBox}>
          ...
          <Text style={styles.infoItem}>🏢 Perusahaan: {po?.company_name ?? '-'}</Text>
          <Text style={styles.infoItem}>🏭 Business Area: {po?.sap_business_area_id ?? '-'}</Text>
...
const styles = StyleSheet.create({
  ...
  content:     { padding: 24 },
  poCard: {
    backgroundColor: '#1e1b4b', borderRadius: 14, padding: 18, marginBottom: 24,
  },
  poCardLabel: {
    fontSize: 10, color: '#a5b4fc', fontWeight: '700',
    letterSpacing: 1.5, marginBottom: 6,
  },
  poNumber:  { fontSize: 20, fontWeight: '700', color: '#ffffff', marginBottom: 4 },
  ...
  amountTotal:      { borderTopWidth: 1, borderTopColor: 'rgba(255,255,255,0.2)', paddingTop: 8, marginTop: 2 },
  ...
  companyBadge:    { backgroundColor: 'rgba(99,102,241,0.3)', paddingHorizontal: 8, paddingVertical: 3, borderRadius: 6 },
  ...
  prefixText: {
    paddingHorizontal: 14, fontSize: 15, color: '#6b7280', fontWeight: '600',
    borderRightWidth: 1, borderRightColor: '#e2e8f0', paddingVertical: 14,
  },
  inputWithPrefix: {
    flex: 1, paddingHorizontal: 14, paddingVertical: 14, fontSize: 15, color: '#111827',
  },
  selectBtn: {
    backgroundColor: '#ffffff', borderWidth: 1, borderColor: '#e2e8f0',
    borderRadius: 10, paddingHorizontal: 16, paddingVertical: 14,
  ...
  infoTitle: { fontSize: 12, fontWeight: '700', color: '#1d4ed8', marginBottom: 8 },
