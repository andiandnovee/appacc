...
export default function ScanPOScreen() {
  ...
  const handleSelectPO = () => {
    if (!result) return;
    navigation.navigate('ReceiptForm', { po: result });
  };
  ...
  return (
    <SafeAreaView style={styles.container}>
      <View style={styles.header}>
        ...
        <Text style={styles.headerTitle}>Scan / Cari PO</Text>
        <View style={{ width: 80 }} />
      </View>
      ...
      <ScrollView contentContainerStyle={styles.content}>
        ...
        <View style={styles.cameraBox}>
          ...
          <Text style={styles.cameraText}>Kamera OCR</Text>
          <Text style={styles.cameraSubText}>Tersedia setelah build APK</Text>
        </View>
        ...
        {searched && !isSearching && (
          <View style={styles.resultsSection}>
            {result ? (
              <TouchableOpacity style={styles.poCard} onPress={handleSelectPO}>
                ...
                {result.items?.map((item, i) => (
                  <View key={i} style={styles.itemRow}>
                    <Text style={styles.itemNo}>Item {item.item_no}</Text>
                    <Text style={styles.itemValue}>
                      Rp {Number(item.net_value).toLocaleString('id-ID')}
                    </Text>
                  </View>
                ))}
...
const styles = StyleSheet.create({
  ...
  poVendor: { fontSize: 14, color: '#374151', marginBottom: 12 },
  itemRow: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    paddingVertical: 6,
  ...
  itemNo: { fontSize: 13, color: '#6b7280' },
  itemValue: { fontSize: 13, color: '#111827', fontWeight: '600' },
  poFooter: {
    flexDirection: 'row',
    justifyContent: 'space-between',
    alignItems: 'center',
