  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setError(
        isEdit
          ? "Gagal mengubah role."
          : "Nama role sudah ada atau tidak valid.",
      );
    } finally {
      setLoading(false);
    }
...
function RolesTab({ roles, setRoles, toast }) {
  ...
  const handleDelete = async (role) => {
    if (role.users_count > 0) {
      toast({
        variant: "warning",
        title: `Role "${role.name}" masih dipakai ${role.users_count} user.`,
      });
      return;
    }
            ...
            {filtered.length === 0 ? (
              <tr>
                <td colSpan={4}>
                  <EmptyState
                    ...
                    description="Buat role baru untuk memulai."
                  />
                </td>
              </tr>
            ) : (
              filtered.map((role) => (
                <tr key={role.id} className={styles.row}>
                  <td>
                    <Badge variant={roleVariant(role.name)} pill>
                      {role.name}
                    </Badge>
                  </td>
                  <td>
                  ...
                  <td style={{ textAlign: "center" }}>
                    <Badge
                      variant={role.users_count > 0 ? "primary" : "default"}
                      size="sm"
                    >
                      {role.users_count}
                    </Badge>
                  ...
                  <td>
                    <div className={styles.actions}>
                      ...
                      <Button
                        ...
                        disabled={
                          role.name === "superadmin" || role.users_count > 0
                        }
                        title={
                          role.users_count > 0
                            ? "Masih ada user dengan role ini"
                            : ""
                        }
                      />
                    </div>
...
export default function UserManagement() {
  ...
  const tabs = [
    ...
    },
    {
      ...
      content: (
        <RolesTab roles={allRoles} setRoles={setAllRoles} toast={addToast} />
      ),
    },
  ];
