...
function VendorFormModal({ vendor, onClose, onSaved }) {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setErrors({ general: isEdit ? 'Gagal menyimpan perubahan.' : 'Gagal menambah vendor. Kode vendor mungkin sudah ada.' })
    } finally {
      setLoading(false)
    }
  }
  ...
  return (
    <Modal isOpen onClose={onClose} size="md">
      <Modal.Header
        ...
        subtitle={isEdit ? `Kode: ${vendor.vendor_code}` : 'Isi data vendor dengan lengkap'}
      />
      <Modal.Body>
        <div className={styles.formGrid}>
          ...
          <div className={styles.formField}>
            ...
            <select
              className={`${styles.select} ${errors.service_type ? styles.selectError : ''}`}
            ...
            {errors.service_type && (
              <p className={styles.errorText}>{errors.service_type}</p>
            )}
          </div>
    ...
    <Modal isOpen onClose={onClose} size="sm">
      ...
      <Modal.Body>
        <p className={styles.confirmText}>
          Vendor <strong>{vendor.name}</strong> ({vendor.vendor_code}) akan dinonaktifkan
          dan tidak bisa dipilih di transaksi baru. Data lama tetap aman.
        </p>
      </Modal.Body>
      <Modal.Footer>
        <Button variant="ghost" onClick={onClose} disabled={loading}>Batal</Button>
...
export default function VendorManagement() {
  ...
  const fetchVendors = useCallback(() => {
    ...
    api(`/vendors?${params}`)
      ...
      .catch(() => addToast({ variant: 'danger', title: 'Gagal memuat data vendor.' }))
  ...
  const handleSaved = (data, isEdit) => {
    if (isEdit) {
      ...
      addToast({ variant: 'success', title: `Vendor "${data.name}" diperbarui.` })
    } else {
      fetchVendors()
      addToast({ variant: 'success', title: `Vendor "${data.name}" ditambahkan.` })
    }
  ...
  const handleDeactivate = async () => {
    ...
    try {
      ...
      setVendors((prev) =>
        prev.map((v) => v.id === deactTarget.id ? { ...v, is_active: false } : v)
      )
      addToast({ variant: 'info', title: `${deactTarget.name} dinonaktifkan.` })
      setDeact(null)
    } catch {
    ...
    <div className={styles.page}>
      ...
      <div className={styles.pageHeader}>
        <div>
          ...
          <p className={styles.pageSubtitle}>Kelola data vendor dan rekanan perusahaan</p>
        </div>
        <Button
            ...
            {loading
              ...
              : vendors.map((v) => (
                <tr key={v.id} className={styles.row}>
                  <td>
                    <span className={styles.vendorCode}>{v.vendor_code}</span>
                  </td>
                  <td>
                  ...
                  <td className={styles.muted}>{serviceLabel(v.service_type)}</td>
                  <td>
                    <Badge
