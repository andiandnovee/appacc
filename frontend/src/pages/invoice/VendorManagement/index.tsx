...
export default function VendorManagement() {
  ...
  const handleStatusToggle = useCallback(
    async (vendor, isActive, refetch) => {
      ...
      try {
        ...
        addToast({
          variant: "success",
          title: `Vendor "${vendor.name}" ${isActive ? "diaktifkan" : "dinonaktifkan"}.`,
        });
        refetch();
      } catch (err) {
  ...
  const handleForceDelete = useCallback(
    async (vendor, refetch) => {
      if (!confirm(`Hapus permanen "${vendor.name}"?`)) return;
      try {
        await api.delete(`/vendors/${vendor.id}/force-delete`);
        addToast({
          variant: "success",
          title: `Vendor "${vendor.name}" dihapus permanen.`,
        });
        refetch();
      } catch (err) {
        addToast({
          variant: "danger",
          title: "Gagal menghapus vendor permanen.",
        });
      }
    },
    [addToast],
    ...
    () => [
      {
        ...
        sortable: true,
        render: (row) => (
          <span className={styles.vendorCode}>{row.sap_id || "—"}</span>
        ),
      },
      {
        key: "name",
        ...
        render: (row) => (
          <div>
            ...
            {row.address && (
              <p className={styles.vendorAddress}>{row.address}</p>
            )}
          </div>
        ),
      },
      {
        ...
        sortable: true,
        render: (row) => (
          <span className={styles.muted}>{row.npwp || "—"}</span>
        ),
      },
      {
        key: "service_type",
        ...
        sortable: true,
        render: (row) => (
          <span className={styles.muted}>{serviceLabel(row.service_type)}</span>
        ),
      },
      {
        key: "pph_type",
        ...
        render: (row) => (
          <span className={styles.muted}>
            {row.pph_type ? `${row.pph_type}% (${row.pph_rate}%)` : "—"}
          </span>
        ),
      },
      {
        ...
        render: (row) => (
          <div className={styles.statusCell}>
            <Toggle
              ...
              size="sm"
              variant={row.deleted_at === null ? "success" : "danger"}
              label={row.deleted_at === null ? "Aktif" : "Tidak Aktif"}
            />
          </div>
        ),
      },
          ...
          <div className={styles.actions}>
            ...
            {row.deleted_at !== null && (
              <Button
                ...
                onClick={() =>
                  handleForceDelete(row, tableRef.current?.refetch)
                }
              >
                Hapus Permanen
              </Button>
  ...
  return (
    <div className={styles.page}>
      ...
      <Table
        ...
        dataKey="data"
        pageSize={15}
        exportName="vendors_export"
        title="Daftar Vendor"
        searchable={true}
        selectable={false}
        defaultParams={{ trash_filter: trashFilter }}
        serverSide={true}
      />
      {formTarget !== null && (
        <VendorFormModal
          vendor={formTarget.id ? formTarget : null}
