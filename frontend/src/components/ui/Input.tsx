...
// ── Focus next focusable element on Enter ──────────────────────────────────
function focusNextOnEnter(
  e: React.KeyboardEvent<HTMLInputElement | HTMLTextAreaElement>,
) {
  if (e.key !== "Enter") return;

  // Textarea: Enter + Shift = newline biasa, Enter saja = pindah focus
...
const Input: React.FC<InputProps> = (props) => {
  ...
  if (type === "textarea") {
    ...
    const { onKeyDown: parentOnKeyDown, ...restWithoutKeyDown } = rest;
  ...
  return (
    <div className={styles.wrapper}>
      ...
      <div className={styles.container}>
        {resolvedIconLeft && (
          <span className={styles.iconLeft}>{resolvedIconLeft}</span>
        )}
        <input
          id={id}
          type={resolvedType}
        ...
        {resolvedIconRight && !hasRightControl && (
          <span className={styles.iconRight}>{resolvedIconRight}</span>
        )}
        {type === "password" && (
          <Button
            variant="ghost"
...
const NumberInput: React.FC<NumberInputProps> = ({
  ...
  iconLeft,
  iconRight,
  onKeyDown: parentOnKeyDown, // ← tambah ini
  ...rest
}) => {
