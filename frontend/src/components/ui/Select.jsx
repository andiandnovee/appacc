...
function AsyncSelect({
  ...
  const { endpoint, searchParam = "search", filters = {}, limit = 5 } = fetchOptions;

  // ✅ 3. fetchData and fetchItemById (using useCallback)
  const fetchData = useCallback(async (search = "", off = 0) => {
    if (!endpoint) return;
    setIsLoading(true);
    try {
      ...
      const data = res.data || [];
      const mapped = data.map(item => ({ value: item.id, label: item.name || item.label }));
      setDropdownOptions(prev => off === 0 ? mapped : [...prev, ...mapped]);
  ...
  const handleSelectOption = useCallback((val) => {
    const selected = dropdownOptions.find(opt => String(opt.value) === String(val));
  ...
  const handleKeyDown = useCallback((e) => {
    ...
    switch (e.key) {
      case 'ArrowDown':
        ...
        setHighlightedIndex(prev => (prev === null ? 0 : Math.min(prev + 1, dropdownOptions.length - 1)));
        break;
      case 'ArrowUp':
        e.preventDefault();
        if (dropdownOptions.length === 0) break;
        setHighlightedIndex(prev => (prev === null ? dropdownOptions.length - 1 : Math.max(prev - 1, 0)));
        break;
      case 'Enter':
        e.preventDefault();
  ...
  const handleScroll = useCallback((e) => {
    const el = e.target;
    if (el.scrollHeight - el.scrollTop <= el.clientHeight + 50 && hasMore && !isLoading) {
  ...
  useEffect(() => {
    ...
    const found = dropdownOptions.find(opt => String(opt.value) === String(value));
    if (found) {
  ...
  useEffect(() => {
    ...
    const hasLabel = dropdownOptions.some(opt => String(opt.value) === String(value));
    ...
    const fetchById = async () => {
      try {
        ...
        setDropdownOptions(prev => {
          const exists = prev.some(opt => String(opt.value) === String(item.id));
  ...
  const inputClass = [styles.asyncInput, styles[size], error ? styles.error : "", isOpen ? styles.open : ""].filter(Boolean).join(" ");
  const dropdownClass = [styles.asyncDropdown, isOpen ? styles.visible : ""].filter(Boolean).join(" ");
